...
class ClassroomReservationSystem:
            ...
            for lecture in root.findall('Lecture'):
                ...
                try:
                    ...
                    for time_code, room in zip(expanded_times, rooms):
                        print(f"[XML 처리] 원본 강의실: {room}") # 추가 
                        # 건물-호실 분리 로직
                        match = re.match(r"^([가-힣A-Za-z]+?)\-?(.*)$", room)
                        if match:
                            building_part, room_part = match.groups()
                            print(f"  → 분할 결과: 건물={building_part}, 호수/이름={room_part}")  # 분할 결과 확인
                            building = self.building_code_map.get(building_part, building_part)
                            room_number = re.findall(r'\d+', room_part)
                            room_number = room_number[0] if room_number else room_part
                        else:
                            building = room
                            print(f"  ❌ 정규식 매칭 실패: {room}") #실패시 문구 출력력
    ...
    def parse_time_code(self, time_code, reference_date=None, days_ahead=30):
        """
        숫자/문자 교시 통합 처리 파서
        ...
        try:
            ...
            for part in period_str.split(','):
                ...
                if '-' in part:
                    ...
                    periods.append(part)
            print(f"[시간 파싱] 코드: {time_code}, 기준일: {reference_date}")
    ...
    def is_conflict(self, new_entry):
        """강화된 충돌 검사 로직"""
        # 건물명 정규화
        new_building = self.building_code_map.get(new_entry['building'], new_entry['building']).replace(' ', '').strip()
        new_building = new_building.replace(' ', '').strip()
        new_room = self.parse_room_number(new_entry['room']).strip()
        ...
        for entry in self.lecture_data + self.website_data + self.manual_data:
            # 건물명 정규화
            entry_building = self.building_code_map.get(entry['building'], entry['building']).strip().replace(' ', '')
            entry_room = self.parse_room_number(entry['room']).strip()
            entry_building = self.building_code_map.get(
                entry['building'], 
                entry['building']
            ).replace(' ', '').strip()
            entry_room = re.sub(r'\D', '', entry['room'])
            # 건물 & 호실 비교
            if entry_building != new_building or entry_room != new_room:
                ...
                #print(f"[DEBUG] {entry['name']} | {entry['building']}-{entry['room']} | {entry['start']} ~ {entry['end']}")# 추가
                #print(f"[DEBUG] 비교 대상: {entry['name']} | {entry_building}-{entry_room} | {entry['start']} ~ {entry['end']}")# 추가
                print(f"\n[충돌 검사] 비교 대상: {entry_building}-{entry_room} vs {new_building}-{new_room}")
                print(f"  엔트리 시간: {entry['start']} ~ {entry['end']}")
                print(f"  신청 시간: {new_entry['start']} ~ {new_entry['end']}")
                continue
            # 시간 비교
            if entry_building == new_building and entry_room == new_room:
